...
This is a small internal knowledge-base bot for a streaming team, built on Infrai so the data path is plain and the failure modes are visible. A note follows an asset from ingestion through processing and creator delivery, and the example keeps that decision in view: fetch notes for one creator, rerank them, then return the top delivery instruction.

Infrai supplies embeddings and vector search behind one OpenAI-compatible `base_url`; the same bearer key is used for the vector and rerank calls. The Python client decodes the `{ok, data, error, metadata}` envelope before it treats a response as successful, and it retries rate limits with backoff instead of pretending they do not happen.
...
`MediaKnowledgeBot.ingest` turns domain notes into vectors. `answer` computes a query embedding (the vector query accepts an embedding, not raw text), filters by creator, and asks `ai.rerank` to select one result. `src/knowledge_bot.py` is the whole service boundary on purpose; `run_demo.py` is the runnable path.
...
The fake client keeps that assertion deterministic and avoids a network call, which is usually what you want when you are checking business logic instead of transport.
